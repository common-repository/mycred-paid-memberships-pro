...
if ( ! class_exists( 'myCred_purchase_membership_hook' ) ) :
	class myCred_purchase_membership_hook extends myCRED_Hook {
		...
		**/
		public function run() {
			add_action( 'pmpro_after_checkout', array($this, 'is_order_new'),10,2);
		}
		
		
		
		public function  is_order_new($user_id,$order){
			
			if($this->my_pmpro_is_order_renewal($order))
				return 'Yes';
			else
				$this->myCred_purchase_membership_save_entry($user_id,$order);
		...
		*	myCred save entry
		**/
		public function myCred_purchase_membership_save_entry($user_id, $order){
						...
						if(!empty($pmp_form_id)):
							foreach($pmp_form_id as $key => $val):
								...
								if($val == $form_id){
									...
									if($response == true){
										mycred_add('mycred_pmp_purchase_membership',$user_id, $creds, $log.' '.$give_form_title,$form_id,$ref_type,$type);
									}
								}else if($val == 999999){
									$response = $this->get_user_limit($limit,$user_id,$type);
									if($response == true){	
